...
This analysis of spoken data came about almost by accident: at the time, we were working on English modal auxiliaries, trying to figure out what makes people distinguish between verbs of similar meaning such as *must* or *have to*. As a side project, I was also exploring a new data source that provided word duration information.
...
(tl;dr, I found answers to the first two questions as described below, but the third question turned out too difficult to measure given the priorities of the projects I have been working on since.)

These properties hadn't been studied for modals so far at all, probably due to lack of data. Differences wouldn't be surprising — homophones like *time* and *thyme* or verb vs. noun uses like *to snow* & *the snow* differ in duration. Usually it's the more frequent and/or more 'basic' meaning that is shorter.[^1]
...
The two meanings occur in relatively distinct lexico-grammatical environments. For example, if followed by a perfect (*That must <u>have been</u> difficult*), the meaning is nearly always epistemic, whereas if followed by a main verb (*We must <u>clean</u> up this mess*), the meaning is usually deontic. As linguistic environments are very reliable cues to the meaning, we must control for frequency of occurrence with adjacent words in the analysis to avoid attributing effects to meaning that are in fact just due to greater likelihood of co-occurrence.

### The question(s)

**In this project, I was interested in whether the two meanings also differ in duration, which has theoretical implications for word storage, but potentially also for lexical change in grammar.**

In general, duration differences are due to:

* Frequency of use in general
* Frequency of co-occurrence with surrounding words
* Emphasis: putting emphasis on words will lead to longer duration
* Speech rate: if the conversation is fast-paced, words will of course be spoken more quickly

Frequency properties are due to expectation and ease of articulation: words that are more common or often co-occur with neighbouring words are more easily retrieved from memory. Hence, less effort is required in their articulation. This has been shown for cases such as *time* vs. *thyme* or *but* vs. *butt*, where the former are vastly more common than the latter.

However, the two meanings of *must* are roughly equally frequent. We measure frequency as number of times they occur in large texts collections (corpora). Differences in *must* frequency depend on the composition of the data base: deontic *must* is more likely in formal situations, while epistemic *must* is characteristic of informal and/or spoken-like discourse. In any case, their frequency differences are on the same order of magnitude where frequency effects are probably negligible.
...
For this kind of analysis, we need a fairly large database (called 'corpus'). This comes with the trade-off that measuring word duration must be automatic and is thus coarse-grained; also the audio quality of large data sets can be very low by traditional standards in phonetics.

Nevertheless, the NewsScape corpus is a very interesting resource for many corpus-linguistics tasks. ([See the RedHenLab for more info.](https://www.redhenlab.org/home)) The [NewsScape2016](https://multimodalcorpora.org/web/) corpus houses corpus-linguistically processed data from US Network television. Closed captions were force-aligned with the audio - this returns annotations of the beginning and end of words, from which the words' duration in centiseconds could be determined.

The data can be extracted via a CQL-based query that is common in corpus linguistics. I picked my data points for this analysis semi-manually from a list of 20,000 uses of *must* in NewsScape2016. The exact procedure and code will be provided at a later point, but basically I excluded tokens if they have substantial pauses before or after *must* or when no speech rate could be determined due to missing context or failed forced-alignment.

I then manually checked ~2,000 examples against their original video using [RapidAnnotator](https://www.rapidannotator.org/). This served two purposes: (a) I removed unusable data such as erroneous force-alignment, transcription, or music, and (b) a colleague and myself annotated the *must* meaning and whether the use carried particular emphasis. In a final step, I also calculated speechrate (using {nsyllable}) and transitional probabilities.
...
The snippet shows 10 random examples of data points to illustrate the data structure:
